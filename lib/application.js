...
var logMiddleware = function (config){
    return function *(next){
        config.logid = this.request.query.logid;
        this.log = log(config);
        yield next;
    }
};
...
app.run = function (port, callback) {
    ...
    if (enable(configs, 'mvc')) {
        configs.mvc = configs.mvc || {};
        this.use(middlewares.mvc.middleware(configs.mvc, this));
    }
    if (enable(configs, 'views')) {
        configs.views = configs.views || {};
        this.use(middlewares.views(configs.views));
    }
    if (enable(configs, 'router')) {
        configs.router = configs.router || {};
