...
const bodyParsers = require('koa-body-parsers');
...
const parsers = module.exports = function (app, config) {
    /**
    ...
    app.context.receive = app.request.receive = function * () {
        ...
        let accepts = [].slice.call(arguments);
        let options = {};
        ...
        let type = this.is.apply(this, accepts);
        if (!type) {
            return this.throw('Only accepts ' + accepts.join(' '), 405);
        }
        let mainType = makeMainType(type);
        let receiveConfig = extend(true, {}, config[mainType]);
...
parsers.middleware = function (config) {
    config = config || {};
    let autoParse = ['urlencoded'];
    if(Array.isArray(config.autoParse)) {
        autoParse = [];
        config.autoParse.forEach(function (autoParseItem) {
            if ('string' !== typeof autoParseItem) {
                throw new Error('Option of "autoParse" in config "parsers" should be an array of string!');
            } 
            autoParse.push(autoParserItem);
        });
    }
    return function * (next) {
        let type;
        if (type = this.is.apply(this, autoParse)) {
            yield this.receive(type);
        }
        yield next;
    }
};
...
function init (config) {
    config = config || {};
    let configForAll = config['*'];
    delete config['*'];
    let typeList = extend(true, {}, defaultTypeList);
    if (configForAll) {
...
function makeMainType (type) {
    let mainType = stringTypes[type];
    if (!mainType) {
        for (let i = 0; i < regexpTypes.length; i++) {
            let item = regexpTypes[i];
            let regexp = item.regexp;
...
receivers.multipart = function * (ctx, config, type, mainType) {
    ...
    let parts = request.parts(config);
    let kv = {};
    let part;
    let paramCount = 0;
    let fileCount = 0;
    while (part = yield parts) {
        if (Array.isArray(part)) {
            paramCount++;
            let key = part[0] || 'param_' + paramCount;
            let value = part[1];
            kv[key] = value;
        }
        else {
            fileCount++;
            let key = part.fieldname || 'file_' + fileCount;
            let value = {
...
function * save (ctx, saveAs, readable) {
    ...
    if (saveAs instanceof Function) {
        ...
        savePath = saveAs;
        for (let propname in readable) {
            let prop = readable[propname];
